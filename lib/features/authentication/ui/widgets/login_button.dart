      ...
      child: ElevatedButton(
        onPressed: () async {
          if (Validator.trySubmit(formKey)) {
            try {
              await firebaseAuthService.instance.signInWithEmailAndPassword(email: emailController.text, password: passwordController.text);

              // Callback to show user an error message if the user is not found
              userNotFoundCallback(false);

              final String userid = firebaseAuthService.instance.currentUser!.uid;
